...
def test_name_matches_a_substring ( fund, employee_client, employee_headers ):
    assert names ( search ( employee_client, employee_headers, name = "Gornji" ) ) == [ "Villa Gornji Milanovac" ]
...
def test_category_matches_inside_the_array ( fund, employee_client, employee_headers ):
    assert names ( search ( employee_client, employee_headers, category = "luxury" ) ) == [ "Ferrari F.40", "Villa Gornji Milanovac" ]
    assert names ( search ( employee_client, employee_headers, category = "vehicles" ) ) == [ "Ferrari F.40", "Zastava F140" ]
...
def test_selling_date_keeps_only_earlier_sales_and_drops_unsold_assets ( fund, employee_client, employee_headers ):
...
def test_a_nested_info_filter_compares_the_right_field ( fund, employee_client, employee_headers ):
    response = search ( employee_client, employee_headers, info_filters = [
        { "field": "engine.power", "operator": "gt", "value": 100 }
    ] )
...
def test_an_info_filter_on_a_string_field ( fund, employee_client, employee_headers ):
    response = search ( employee_client, employee_headers, info_filters = [
        { "field": "location.city", "operator": "eq", "value": "Gornji Milanovac" }
    ] )
