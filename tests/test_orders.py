...
def test_approving_a_buy_order_writes_the_asset_and_clears_redis(
    employee_client, employee_headers, director_client, director_headers, assets, orders
):
    buy(employee_client, employee_headers)
    ...
    assert before <= asset["buying_date"].replace(tzinfo=UTC) <= after


def test_the_buying_date_is_utc_not_local_time(
    employee_client, employee_headers, director_client, director_headers, assets, orders
):
    buy(employee_client, employee_headers)
    ...
    horizon = datetime.now(UTC) + timedelta(minutes=30)

    assert assets.count_documents({"buying_date": {"$gt": horizon}}) == 0


def test_rejecting_a_buy_order_writes_nothing(
    employee_client, employee_headers, director_client, director_headers, assets, orders
):
    buy(employee_client, employee_headers)
...
def test_approving_a_sell_order_updates_the_asset(
    employee_client, employee_headers, director_client, director_headers, assets, orders
):
    identifier = insert_asset(assets)
    ...
    assert before <= asset["selling_date"].replace(tzinfo=UTC) <= after
    assert asset["buying_price"] == 100000
    assert assets.count_documents({}) == 1


def test_rejecting_a_sell_order_leaves_the_asset_unsold(
    employee_client, employee_headers, director_client, director_headers, assets, orders
):
    identifier = insert_asset(assets)
...
def test_a_decision_leaves_the_other_orders_alone(
    employee_client, employee_headers, director_client, director_headers, orders
):
    buy(employee_client, employee_headers)
    buy(employee_client, employee_headers, name="Zastava 101")
...
def test_decision_rejects_a_malformed_uuid(director_client, director_headers, value):
    response = director_client.post(
        "/decision", json={"uuid": value, "approved": True}, headers=director_headers
    )

    assert response.json == {"message": "Invalid uuid."}


def test_decision_rejects_a_well_formed_uuid_that_waits_for_nothing(
    director_client, director_headers
):
...
def test_the_uuid_is_fully_validated_before_approved_is_looked_at(
    director_client, director_headers
):
...
def test_decision_reports_an_absent_approved(
    employee_client, employee_headers, director_client, director_headers, orders
):
    buy(employee_client, employee_headers)
    ...
    response = director_client.post(
        "/decision", json={"uuid": order_uuid}, headers=director_headers
    )
...
def test_decision_rejects_anything_that_is_not_a_real_boolean(
    employee_client, employee_headers, director_client, director_headers, orders, assets, value
):
    buy(employee_client, employee_headers)
...
def test_decision_needs_a_director_token(director_client, employee_headers):
    assert (
        decide(director_client, employee_headers, uuid=str(uuid.uuid4()), approved=True).status_code
        == 401
    )
